...
- 说明:
    - protocol（协议）
        ...
        - 明文传输 早期的展示性的,宣传性的网站,无需加密
        - https 通过安全的 HTTPS 访问该资源。 格式 `HTTPS://`
        - 加密传输 适用于更多业务,特别是由资金往来业务
...
urlpatterns = [
    ...
    #第二个参数表示当用户输入这个url后,我们使用哪个视图函数去处理.
...
- 练习:
    ...
    - 两个url可以对应同一个视图函数;path/re.path转换器传参时是关键字传参,与为位置无关
...
- HTTP1.1 请求详述
    ...
    | 2 | HEAD | 类似于get请求，只不过返回的响应中没有具体的内容，用于获取报头 **获取包头** |
    ...
    | 7 | OPTIONS | 允许客户端查看服务器的性能。**协商请求项目时,细讲** |
    | 8 | TRACE | 回显服务器收到的请求，主要用于测试或诊断。**测试路由** |
