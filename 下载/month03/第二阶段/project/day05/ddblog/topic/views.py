...
class TopicView(View):
    ...
    def post(self,request,author_id):
        ...
        #  检察权限的值一定是public或private
        category = json_obj['category']
        if category not in ['tec','no-tec']:
            result = {'code':10300,'error':'分类错误错误!'}
    ...
    def get(self,request,author_id):
        ...
        try:
            ...
            return JsonResponse(result)
        #2 获取访问者身份这个对象(有可能是游客,有可能是某一登录用户)
        ...
        # 文章列表页
            ...
            if visitor_name==author_id:
                # 需要分类
                if fiter_category:
                    author_topic = Topic.objects.filter(user_profile_id = author_id,
                                                        category = category)
                else:
                    author_topic = Topic.objects.filter(user_profile_id=author_id)
    ...
    def make_topic_res(self,author,author_topic,is_self):
        ...
        result['data']['created_time'] = author_topic.created.strftime('%Y-%m-%d %H:%M:%S')
        #文章列表中上一篇下一篇
        if is_self:
